...
test('writers room shows proposals/parked only, not developing or canon items',()=>{
  ...
  const items=[{id:'idea',status:'idea'},{id:'parked',status:'parked'},{id:'dev',status:'developing'},{id:'canon',status:'canon'}];
...
test('related-character suggestions prefer characters already connected in scenes',()=>{
  const data={characters:[{id:'a',title:'A',tags:['family']},{id:'b',title:'B',tags:['family']},{id:'c',title:'C',tags:['enemy']}],scenes:[{characters:['a','b']},{characters:['a','b']}],relationships:[]};
  assert.equal(generator.relatedCharacters(data,'a')[0].id,'b');
});
...
test('writers room generator exposes a broad set of scene idea modes',()=>{
  ...
  for(const mode of ['related','relationship','theme','conflict','comedy','flashback','secret','family','surprise','mentor','rivalry','dilemma','quiet','training','aftermath','misunderstanding','promise','foreshadow','school','journey','villain','legacy'])assert.match(ui,new RegExp(`value="${mode}"`));
});
...
test('AI failures preserve backend detail and show a persistent generator error',()=>{
  const ai=fs.readFileSync('raika-ai-client-v2.js','utf8');
...
test('writers room loads the detailed AI client before generator actions',()=>{
  const html=fs.readFileSync('raika-writers-room.html','utf8');
  assert.match(html,/raika-ai\.js[^]*raika-ai-client-v2\.js[^]*raika-generator-actions\.js/);
});
