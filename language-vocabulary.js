(function(){
  ...
  function renderHeader(){
    ...
    $('vocab-language-switch').innerHTML=Object.entries(B.LANGUAGES).map(([code,item])=>`<a class="study-switch ${code===lang?'active':''}" href="language-vocabulary.html?lang=${code}&topic=${activeTopic}">${item.code} · ${item.name}</a>`).join('');
  }
  ...
  function searchEntries(query){
    ...
    allEntries().forEach(entry=>{
      ...
      const text=[entry.word.he,d.primary,d.secondary,entry.topic.name].filter(Boolean).join(' ').toLowerCase();
  ...
  $('vocab-topic-nav').addEventListener('click',event=>{
    ...
    history.replaceState(null,'',`language-vocabulary.html?lang=${lang}&topic=${activeTopic}`);
  });
