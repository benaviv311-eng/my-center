(function(root,factory){
  ...
  function wordObject(lang,row,heOverride){return {lang,id:row[0],target:row[1],pron:row[2]||'',he:heOverride??row[3]};}
  function sentenceObject(lang,row,heOverride){return {lang,id:row[0],target:row[1],pron:row[2]||'',he:heOverride??row[3]};}
  ...
  function sharedWordHebrew(id){return findWord('ar',id)[3];}
  function sharedSentenceHebrew(id){return findSentence('ar',id)[3];}
  ...
  function challengeVariant(lang,concept,seed){
    const correct=wordObject(lang,findWord(lang,concept),sharedWordHebrew(concept));
  ...
  function variantsFor(type,index,seed){
    ...
    if(type==='word'){
      ...
      const hebrew=sharedWordHebrew(concept);
      CODES.forEach(lang=>{variants[lang]={item:wordObject(lang,findWord(lang,concept),hebrew)};});
      return {variants,hebrew,concept};
    }

    if(type==='sentence'){
      const concept=pick(SHARED_SENTENCE_IDS,variantHash);
      const hebrew=sharedSentenceHebrew(concept);
      CODES.forEach(lang=>{variants[lang]={item:sentenceObject(lang,findSentence(lang,concept),hebrew)};});
      return {variants,hebrew,concept};
    }
    ...
    if(type==='challenge'){
      ...
      return {variants,hebrew:sharedWordHebrew(concept),concept};
    }
  ...
  function wordItemsForLang(lang,concepts){
    return concepts.map(id=>wordObject(lang,findWord(lang,id),sharedWordHebrew(id)));
  }
  ...
  function buildFourLanguageMiniGame({type='flashcards',seed='four-game'}={}){
    ...
    if(safeType==='sentence-builder'){
      ...
      const hebrew=sharedSentenceHebrew(sentenceId);
      CODES.forEach(lang=>{
        const sentence=sentenceObject(lang,findSentence(lang,sentenceId),hebrew);
      ...
      return {id,type:safeType,lang:'four',variants,prompt:hebrew};
    }
    ...
    return {id,type:safeType,lang:'four',variants,prompt:sharedWordHebrew(concepts[0]||SHARED_WORD_IDS[0])};
  }
